...
class TriesControllerTest < ActionController::TestCase
  ...
  test "should create try" do
    assert_difference('Try.count') do
      post :create, try: { amount: @try.amount, multiplicator: @try.multiplicator, player_id: @try.player_id, position: @try.position, total: @try.total }
    end
  ...
  test "should update try" do
    put :update, id: @try, try: { amount: @try.amount, multiplicator: @try.multiplicator, player_id: @try.player_id, position: @try.position, total: @try.total }
    assert_redirected_to try_path(assigns(:try))
  end
